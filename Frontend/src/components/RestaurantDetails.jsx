...
import { IoStar, IoStarHalf, IoStarOutline, IoTime, IoCall, IoLocation, IoArrowBack, IoSearch, IoFilter, IoCart, IoAdd, IoRemove, IoHeart, IoHeartOutline, IoPricetag, IoInformationCircle } from 'react-icons/io5';
...
const RestaurantDetails = ({ restaurantId, cartItems }) => {
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        const response = await fetch("https://king-hub-1.onrender.com/api/restaurants/67c54bc9758356ddea201250");
        if (!response.ok) {
    ...
    return (
      <div className="min-h-screen flex justify-center items-center bg-indigo-50">
        <div className="animate-pulse flex flex-col items-center">
          ...
          <p className="text-indigo-600 font-medium">Loading restaurant details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex justify-center items-center bg-indigo-50">
        <div className="text-center p-8 bg-white rounded-xl shadow-xl max-w-md mx-auto">
          ...
          <h2 className="text-3xl font-bold text-gray-800 mb-3">Restaurant Not Found</h2>
          <p className="text-gray-600 mb-6 text-lg">We couldn't find the restaurant you're looking for.</p>
          <button
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
              ...
              <div className="flex items-center mt-2">
                <div className="flex text-yellow-400">
                  {[...Array(5)].map((_, i) => (
                    <span key={i}>
                      {i < Math.floor(item.rating) ? <IoStar /> : i < Math.ceil(item.rating) && item.rating % 1 !== 0 ? <IoStarHalf /> : <IoStarOutline />}
                    </span>
                  ))}
                </div>
                <span className="ml-2 text-white font-medium text-sm">{item.rating} ({item.reviewCount} reviews)</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                className={`p-2 rounded-full ${favorites.includes(item.id) ? 'text-pink-500 bg-pink-100' : 'text-gray-400 bg-gray-100'} transition-colors`}
              >
                {favorites.includes(item.id) ? <IoHeart className="h-6 w-6" /> : <IoHeartOutline className="h-6 w-6" />}
              </button>
            </div>
            ...
            {item.allergies.length > 0 && (
              <div className="mb-6">
                <h3 className="font-medium text-gray-900 mb-2">Allergen Information</h3>
            ...
            <div className="bg-indigo-50 p-4 rounded-lg mb-6">
              ...
              <div className="flex space-x-1">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className={`h-3 w-6 rounded-full ${i < item.spicyLevel ? 'bg-red-500' : 'bg-gray-200'}`}
                  />
                ))}
                <div className="ml-2 text-gray-600 text-sm">
                  {['Not Spicy', 'Mild', 'Medium', 'Spicy', 'Very Spicy'][item.spicyLevel]}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="divide-y divide-gray-200">
              {days.map((day) => (
                <div
                  key={day}
                  className={`py-3 flex justify-between items-center ${day === today.toLowerCase() ? 'bg-indigo-50 -mx-6 px-6' : ''}`}
                >
              ...
              <div className="flex items-center text-gray-800">
                ...
                <span>
                  {restaurant.openTime ? (
                    ...
                    <span className="font-medium text-gray-600">Currently Closed</span>
                  )}
                </span>
              </div>
            </div>
    ...
    <div className="bg-indigo-50 min-h-screen font-sans">
      {loading ? (
        ...
        <>
            ...
            <div className="absolute bottom-0 left-0 right-0 px-6 py-6 md:px-10 md:py-8 flex items-end">
              <div className="flex items-center">
                ...
                <div className="ml-4 md:ml-6">
                  <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">{restaurant.name || "Restaurant"}</h1>
                  <div className="flex items-center mt-2">
                          ...
                          <span key={i} className="mr-0.5">
                            {ratingValue <= restaurant.rating ?
                              ...
                              ratingValue <= Math.ceil(restaurant.rating) &&
                                ...
                                <IoStarHalf className="text-yellow-400" size={18} /> :
                                <IoStarOutline className="text-yellow-300" size={18} />
                            }
                          </span>
                        );
                      })}
                    ...
                    <span className="ml-2 text-white font-medium">
                      {restaurant.rating?.toFixed(1) || "0.0"} <span className="opacity-80">({restaurant.reviewCount || 0} reviews)</span>
                    </span>
                    <span className="ml-4 text-white font-bold">{restaurant.priceLevel || "$"}</span>
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8 transform transition-all">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex items-center group">
                  ...
                  <div className="ml-4">
                    ...
                    <div className="text-base font-medium text-gray-800">{restaurant.location || "Address unavailable"}</div>
                  </div>
                </div>

                <div className="flex items-center group">
                  <div className="p-3 bg-green-100 rounded-full text-green-600 group-hover:bg-green-600 group-hover:text-white transition-all duration-300"
                  ...
                  <div className="ml-4">
                    ...
                    <div className="text-base font-medium text-gray-800 flex items-center cursor-pointer"
                      onClick={() => setShowHoursModal(true)}
                    >
                      {restaurant.isOpen ?
                ...
                <div className="flex items-center group">
                  <a href={`tel:${restaurant.contact}`} className="p-3 bg-blue-100 rounded-full text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                    <IoCall size={22} />
                  </a>
                  <div className="ml-4">
                    <div className="text-sm text-gray-500 font-medium">Contact</div>
                    <a href={`tel:${restaurant.contact}`} className="text-base font-medium text-gray-800 hover:text-indigo-600 transition-colors">{restaurant.contact || "Phone unavailable"}</a>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t border-gray-100">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-sm text-gray-500 font-medium">Delivery Time</div>
                    <div className="text-base font-medium text-gray-800">{restaurant.deliveryTime || "30-45 minutes"}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-sm text-gray-500 font-medium">Delivery Fee</div>
                    <div className="text-base font-medium text-gray-800">{restaurant.deliveryFee || "$0.00"}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 mb-2 overflow-x-auto scrollbar-hide">
              <div className="flex space-x-2 pb-1">
                {restaurant.menu?.map((category) => (
                  <button
                    key={category.id}
                    className={`whitespace-nowrap px-5 py-2 rounded-full font-medium transition-all ${activeCategory === category.id
          ...
          {/* Cart Slide Panel */}
          {showCart && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex justify-end animate-fadeIn" onClick={() => setShowCart(false)}>
              <div
                ref={cartRef}
                ...
                <div className="flex flex-col h-full">
                  <div className="p-6 border-b border-gray-200">
                    ...
                    <div className="mt-2 text-gray-600">
                      <span className="font-medium text-indigo-600">{restaurant.name || "Restaurant"}</span>
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    {cart.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-gray-600 mb-6 max-w-xs">Add items from the menu to start your order.</p>
                        <button
                          onClick={() => setShowCart(false)}
